...
function observer(Component) {
    ...
    if (!prototype[symbolAutoRuns] && !prototype[symbolReactions])
        return;
    ...
    if (!_onLoad) {
        ...
        prototype.onLoad = function onEnable() {
    ...
    if (!_onDestroy) {
        ...
        prototype.onDestroy = function onDestroy() {
            dispose.call(this);
...
function dispose() {
    ...
    if (disposers)
        disposers.splice(0).forEach((x) => x());
}
function getDisposers(object) {
...
function attach(target, key, storeKey) {
...
export { action, computed, configure, isObservableArray, isObservableMap, isObservableObject, isObservableSet, observable, observer, react, reactor, render, runInAction, toJS };
